...
def analyze_vcp_breakout_trader(
    ...
    *,
    ...
    rs_turn = (close > high.shift(1).rolling(20, min_periods=20).max() * 0.92) & (
        frame["ema21"] > frame["ema21"].shift(3)
    )
    leader_zone = near_high | rs_turn
    ...
    valid_setup_series = setup_series.rolling(5, min_periods=1).sum() >= 1

    vcp_setup = vcp_setup_series.fillna(False).tolist()
    flag_setup = flag_setup_series.fillna(False).tolist()
    setup = setup_series.tolist()
    ...
    condition_map = {
        ...
        "强势/靠近高位": bool(leader_zone.iloc[latest_idx]),
        ...
        "最近 5 日出现准备区": bool(valid_setup[latest_idx]),
    }
    ...
    signal_labels = []
    if vcp_setup[latest_idx]:
        signal_labels.append("VCP")
    if flag_setup[latest_idx] and not vcp_setup[latest_idx]:
    ...
    if fail[latest_idx]:
        ...
        grade = 3
        verdict = "LC 提前买点"
        latest_reasons.append("价格收回 EMA10/EMA21 并贴近短枢轴，形成 Low-Cheat。")
        ...
        grade = 1
        verdict = "准备区延续"
        latest_reasons.append("最近 5 日出现过准备区，但最新一根未完全满足准备区。")
    else:
        grade = 0
        verdict = "不适合新开仓"
    ...
    for idx in range(max(0, len(frame) - 80), len(frame)):
        ...
        if vcp_setup[idx]:
            labels.append("VCP")
        if flag_setup[idx] and not vcp_setup[idx]:
            labels.append("FLAG")
        if lc[idx]:
            labels.append("LC")
        if break_buy[idx]:
    ...
    pivot = latest["pivot"]
    risk_pct = None
    if stop_base is not None and not pd.isna(stop_base) and float(latest["close"]) > 0:
        risk_pct = max((float(latest["close"]) - float(stop_base)) / float(latest["close"]) * 100, 0)
    ...
    return {
        ...
        "setup": {
            ...
            "leader_zone": condition_map["强势/靠近高位"],
        ...
        "risk_plan": {
            ...
            "extension_warning": "出现 EXTEND/伸 时，新仓慎追；已有仓位按 10/21EMA 或前低跟踪。",
        },
        "parameters": {
