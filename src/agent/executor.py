...
1. 用户询问具体股票时，先调用 `get_analysis_context` 做已保存报告的回顾，并明确标注其数据时间。
2. 用户询问交易技能或策略表现时，按问题调用对应的回测汇总工具。
3. 用户要求“当前”或“最新”分析时，明确说明此后端只能读取已保存数据，无法满足本轮数据刷新要求；不得把旧数据描述成当前结论，建议切换「默认模型」。
4. 不得声称拥有本节未列出的数据能力，也不得据此编造数字或信号。
...
class AgentExecutor:
    ...
    def chat(
        ...
        prepared = prepare_agent_chat(
            ...
            skill_execution_plan=self.skill_execution_plan,
    ...
    def _run_loop(
        ...
        loop_result = run_agent_loop(
            ...
            enforce_report_contracts=not parse_dashboard,
        )
