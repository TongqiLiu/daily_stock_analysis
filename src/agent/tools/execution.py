...
def _normalize_guard_stock_code(value: Any, registry: Optional[Any] = None) -> str:
    ...
    # Agent prompts may use Futu/provider form (US.NFLX) while the scope stores
    # the canonical bare US ticker (NFLX); treat both as the same underlying.
    return normalized_text[3:] if normalized_text.startswith("US.") else normalized_text
