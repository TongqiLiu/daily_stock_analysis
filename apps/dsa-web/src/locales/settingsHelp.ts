...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.agent.AGENT_LLM_TRANSIENT_RETRIES': {
    title: 'LLM 传输重试轮数',
    summary: '连接失败或请求超时时，额外重试问股模型链的轮数。',
    usage: '仅重试发生连接、TLS 或超时错误的模型；0 表示关闭额外传输重试。',
    valueNotes: ['建议 2；网络波动明显时可提高到 4，但失败后的等待时间会增加。'],
    impact: ['减少中转端点短时抖动导致的问股失败。'],
    notes: ['鉴权、余额、参数、限流和上下文错误不会触发该重试。'],
  },
  'settings.agent.AGENT_LLM_RETRY_BASE_DELAY_S': {
    title: 'LLM 重试退避基数',
    summary: '问股传输重试的初始等待秒数。',
    usage: '按该值指数退避，例如设为 2 时依次等待 2、4、8 秒，单次最长 30 秒。',
    valueNotes: ['建议 2；设为 0 可立即重试。'],
    impact: ['影响网络故障恢复概率和失败后的总等待时间。'],
    notes: ['所有等待都受 Agent 总超时剩余预算约束。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.agent.AGENT_LLM_TRANSIENT_RETRIES': {
    title: 'LLM Transport Retry Rounds',
    summary: 'Extra Agent model-chain rounds after connection or request timeout failures.',
    usage: 'Only models that failed with connection, TLS, or timeout errors are retried. Set 0 to disable extra transport retries.',
    valueNotes: ['Use 2 normally; raise to 4 on unstable networks at the cost of a longer final failure time.'],
    impact: ['Reduces ask-stock failures caused by brief gateway interruptions.'],
    notes: ['Authentication, quota, parameter, rate-limit, and context errors are not retried here.'],
  },
  'settings.agent.AGENT_LLM_RETRY_BASE_DELAY_S': {
    title: 'LLM Retry Base Delay',
    summary: 'Initial backoff in seconds for Agent transport retries.',
    usage: 'Backoff grows exponentially. A value of 2 waits 2, 4, then 8 seconds, capped at 30 seconds per wait.',
    valueNotes: ['Use 2 normally; set 0 for immediate retries.'],
    impact: ['Affects recovery probability and total wait time during network failures.'],
    notes: ['Every wait remains bounded by the Agent overall timeout budget.'],
  },
  'settings.agent.AGENT_RISK_OVERRIDE': {
