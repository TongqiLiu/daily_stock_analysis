...
def _recent_dividend_index() -> pd.DatetimeIndex:
    """Return four deterministic-in-window dates without coupling tests to wall-clock year."""
    reference = pd.Timestamp.now(tz="America/New_York").normalize()
    return pd.DatetimeIndex(
        [reference - pd.Timedelta(days=days) for days in (270, 180, 90, 1)]
    )
