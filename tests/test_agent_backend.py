...
def test_codex_chat_factory_does_not_construct_litellm_context_adapter() -> None:
    prompt_state = SimpleNamespace(
        skill_instructions="",
        skill_execution_plan="specialist plan",
    ...
    assert executor.skill_execution_plan == "specialist plan"
...
def test_litellm_backend_forwards_cooperative_cancellation() -> None:
    registry = ToolRegistry()
    adapter = _FinalAnswerAdapter()
    cancel_event = threading.Event()
    cancel_event.set()

    wrapped = LiteLLMAgentBackend(registry, adapter).run(
        _request(cancel_event=cancel_event)
    )

    assert wrapped.success is False
    assert wrapped.error_code == "cancelled"
    assert wrapped.error_message == "Agent execution cancelled"
