...
def test_cancelled_litellm_turn_does_not_persist_a_synthetic_failure_note() -> None:
    backend = _failure_backend(
        runtime_owns_loop=False,
        error_code="cancelled",
    ...
    with patch("src.agent.chat_executor.prepare_agent_chat", return_value=prepared), \
         patch("src.agent.chat_executor.conversation_manager.get_or_create"), \
         patch("src.agent.chat_executor.conversation_manager.add_message", return_value=1) as add_message:
        result = _executor(backend).chat(
            "question",
            "session",
            cancel_event=cancel_event,
    ...
    assert result.error_code == "cancelled"
    add_message.assert_called_once_with("session", "user", "question")
