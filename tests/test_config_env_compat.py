...
from src.config import Config, DEFAULT_ALPHASIFT_INSTALL_SPEC, DEFAULT_STOCK_LIST, setup_env
...
class ConfigEnvCompatibilityTestCase(unittest.TestCase):
    ...
    def test_refresh_stock_list_falls_back_to_default_when_empty(self) -> None:
        # An empty STOCK_LIST falls back to the curated DEFAULT_STOCK_LIST so the
        # system stays runnable without explicit config.
        with tempfile.TemporaryDirectory() as temp_dir:
            env_path = Path(temp_dir) / ".env"
        ...
        self.assertEqual(config.stock_list, list(DEFAULT_STOCK_LIST))
    ...
    def test_invalid_numeric_env_values_fall_back_to_defaults(
        ...
        self.assertEqual(config.agent_orchestrator_timeout_s, 900)
