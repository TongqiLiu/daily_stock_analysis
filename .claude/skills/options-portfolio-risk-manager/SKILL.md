...
- **Support / resistance check is mandatory** before new short options — see `options-income-scanner` →「支撑位 / 阻力位（卖 Put·Call 硬性风控）」: near support → avoid new short calls / prefer short puts if assignment OK; near resistance → avoid new short puts / prefer covered calls if willing to trim.
