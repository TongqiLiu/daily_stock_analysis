...
本文档包含自选股智能分析系统的完整配置说明，适合需要高级功能或特殊部署方式的用户。
...
>
> 筹码分布补充：A 股优先使用 Tushare/AkShare 原生筹码接口；美股/港股无等价原生接口时，会基于近 120 日 OHLCV 做 volume-profile 近似估算（用于趋势参考，不等同于交易所原生筹码数据）。
...
P5 后验评估只支持日线可验证的 `1d/3d/5d/10d`，窗口语义是 anchor 后 1/3/5/10 根 `StockDaily` 交易 bar，不复用 `DecisionSignalService._horizon_days()` 的自然日过期语义。`anchor_date` 优先读取 `metadata.market_phase_summary.effective_daily_bar_date`，其次读取 `session_date`，最后使用 `created_at.date()`；解析出的 anchor 当日必须存在 `StockDaily.close`。这样会优先使用分析时已经完成且可见的日线，避免盘中或非交易日前视。动作映射为 `buy/add -> up`、`hold -> not_down`、`reduce/sell/avoid -> not_up`；`watch/alert`、`intraday/swing/long`、缺 anchor 价、forward bars 不足等会写入 `eval_status=unable` 和明确 `unable_reason`。缺 anchor 价、非法 anchor 价、forward bars 不足、缺/非法窗口收盘价属于可恢复 unable，后续默认重跑会在数据补齐后重新评估；非方向动作、不支持 horizon 和缺 anchor date 属于终态 unable，默认保持幂等跳过。自动提取运行时可额外接收 `portfolio_context.quantity`，只把低敏 `holding_state=holding|empty|unknown` 写入 metadata 供后验快照使用，不保存数量、账户或成本。
...
- 📋 **问股对话复制** - 输入框旁的“复制”按钮会把当前用户与 AI 消息整理成 Markdown 后写入剪贴板；空会话时按钮禁用，复制成功后会短暂显示“已复制”
- 🧠 **会话级 Skill 选择** - 问股会话会持久化当前 Skill 选择；刷新或切换会话时恢复各自选择，新会话继续使用服务器默认 Skill
- ⏱️ **日内做T Skill** - 问股策略选择器提供「日内做T」，固定使用本轮3分钟K线计算 EMA20/EMA50、ATR14 与 HH/HL/LH/LL；强趋势少T，箱体才T，高卖要求 LH+EMA20 失守，低接要求支撑+HL+EMA20 reclaim，并校验至少 1 ATR、优选 1.5 ATR 的计划价差
- 🧭 **多策略价格结构** - 多策略联合评分在权威评分区块中额外展示当前价格位置、近端支撑、近端阻力、突破状态与硬失效位；字段来自 `analyze_trend`，数据缺失时明确标记，不用模型估算价格
- 🧮 **期权策略分析** - 问股策略选择器新增「期权策略分析」及「期权组合分析」入口，按真实期权链、IV/HV、成交量、OI 和供应商 Greeks 比较 Call/Put 与价差；具体组合通过 Futu `get_option_strategy_analysis` 获取组合级 Bid/Ask、最大盈亏、盈亏平衡点、盈利概率、Delta 与 Theta。权限或字段缺失时降级为数据缺口，不输出伪造价格或 Gamma
...
| `/api/v1/decision-signals/outcomes` | GET | 分页查询信号后验结果，支持按标的、来源、周期、状态和结果筛选，并返回原始信号审计上下文 |
| `/api/v1/decision-signals/outcomes/stats` | GET | 查询当前后验引擎统计，默认排除 archived 信号，可按 `source_type=agent` 隔离问股样本 |
| `/api/v1/decision-signals/outcomes/ai-review` | POST | 基于确定性统计生成只读 AI 后置复盘，不重算胜负或修改权重 |
...
> 说明：问股的 `multi_strategy_consensus` 是独占元 skill，不能与 Serenity 投研、价值投资等专项 skill 同时执行。Web 改选专项 skill 时会自动取消默认的多策略联合；API/旧客户端若提交冲突组合，后端会丢弃 `multi_strategy_consensus` 并保留专项 skill。多个专项 skill 会按请求顺序生成执行计划，先完成各自声明的必要工具，再逐项输出并综合；本轮最新请求和本轮 skills 选择优先于历史消息中未重申的范围限制。多策略联合的 13 项固定总权重为 10.0，其中价格行为学权重 0.8 并归入结构维度；它使用重要低点、ATR 缓冲保护、强阳 K 和真实 5m/15m VWAP 管理失效与移动止盈。分钟线优先 Longbridge、失败时降级 Futu OpenD，并返回来源、交易日和 as-of；数据过期或缺失时不得使用日线代理。评分工具会校验信号/强度/分数区间，按完整、部分、缺失报告证据覆盖率，并将缺失项从分子分母同时排除；最终答复若篡改工具得分、有效分母、决策或仓位会被运行时拦截重做。若判断第 3 浪候选/启动，报告必须附带日期价位浪型图、替代数浪、确认位、弱化位、硬证伪位和置信度；`get_weekly_history` 在 Futu OpenD 启用时优先读取原生周线，失败后回退到日线聚合，并显式标记来源、当前周未完成状态和回退原因；缩量突破、缺少周线或长期均线空头时不得给高置信度。

> 多策略相对强弱补充：系统按共同交易日计算个股相对大盘及可验证行业基准的 5/20/60 日超额收益，并展示基准代码、来源和截止日期；行业基准缺失或过期时只保留部分证据，不猜测龙头地位。五维方向出现高冲突时，确定性评分会覆盖原始强方向分数，统一输出「证据冲突（观望）」且不建议新增仓位。权威评分区块还会独立展示「仓位与分批执行」：比例仅指计划仓位的 50%/30%/20% 分段，实际金额必须由账户风险预算和止损距离换算；只有首仓盈利且结构确认才允许加仓。1R/2R仅是风险收益观察位，不再默认各减25%；斐波那契位仅是由本轮支撑—阻力推导的回踩观察区，不能在下跌中机械摊平。

> 持仓执行约束：执行门控未通过也禁止持仓加仓，但已有仓位的失效位与保护规则仍显示。压力位与缩量不单独触发减仓，需区分强势延续、整理和突破失败后的空头跟随；硬失效或风险超预算的保护优先。每次回补/净加仓均需按拟成交价、新的有效结构止损、下一阻力重新评估盈亏比（至少1.5）与总风险，不继承旧入场的盈亏比。缺持仓/风险预算时不编造减仓比例；减仓降风险不保证降低成本，总盈亏必须包含已实现、未实现与费用。上述条件不是自动下单逻辑，也不代表胜率已提升。

> 量能口径：`get_volume_analysis`只使用交易日历可验证的已完成日线，返回`data_as_of`、`bar_completion`、剔除的未完成/未来日线数及均量实际样本数；均量不含被比较日。盘中返回的量能通常截至前一已完成交易日，不代表当前盘中量能。日历/日期不可验证时不返回可用于确认的量比，明确降级；冻结历史日期之后的数据不得参与计算。其他技术工具的数据仍应分别检查截止日期，不能将此工具的完成标记推广到所有工具。
