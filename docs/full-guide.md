...
> 多策略相对强弱补充：系统按共同交易日计算个股相对大盘及可验证行业基准的 5/20/60 日超额收益，并展示基准代码、来源和截止日期；行业基准缺失或过期时只保留部分证据，不猜测龙头地位。五维方向出现高冲突时，确定性评分会覆盖原始强方向分数，统一输出「证据冲突（观望）」且不建议新增仓位。权威评分区块还会独立展示「仓位与分批执行」：比例仅指计划仓位的 50%/30%/20% 分段，实际金额必须由账户风险预算和止损距离换算；只有首仓盈利且结构确认才允许加仓，1R/2R 提供分批减仓参考，斐波那契位仅是由本轮支撑—阻力推导的回踩观察区，不能在下跌中机械摊平。
