...
- [修复] 多策略执行门控同步约束持仓加仓与空仓买入，缺少止损/阻力或盈亏比不足时不得因方向高分绕过；禁止新增时仍显示已有持仓保护计划。
- [改进] 仓位管理将1R/2R从固定25%止盈改为结构观察位，取消缩量单独触发减仓；回补/加仓需重新评估成交价、新结构止损、下一阻力与总风险，区分降低风险与做T降本。
- [修复] 问股量能工具按交易日历剔除未完成日线及冻结日期之后的数据，以此前已完成交易日计算均量并返回截止日期；日历或日期无法验证时明确量能待确认。
- [测试] 增加持仓执行门控、硬失效退出、1.5盈亏比边界、条件式止盈及盘中/盘后/历史冻结量能的回归测试；这些测试不代表策略胜率验证。
- [修复] 问股新闻搜索在未配置第三方搜索 API Key 时自动降级到富途公开新闻接口；对 PENG 等歧义代码优先使用公司英文身份并拒绝无直接公司匹配的噪音结果。

- [新功能] 新增独立的问股决策表现中心，支持全部/仅问股来源切换，集中展示 T+1/T+3/T+5/T+10 结算覆盖、方向胜率、区间收益、平均/最大不利波动及关键维度拆分，并可生成只读 AI 后置复盘。
- [改进] 问股决策表现中心新增可筛选的逐笔回测明细，逐条展示分析时点、原始结论与计划价格、日线回测输入、命中结果和无法结算原因，便于审计胜率口径。
- [改进] 单股票问股的明确最终动作会 best-effort 沉淀为结构化决策信号；后验锚点优先使用分析时已完成的有效日线，避免盘前、盘中和非交易日前视偏差，且持久化失败不阻断问股输出。
- [改进] 问股对话在每条用户输入和 AI 输出顶部显示发送或完成时间，便于区分分析时点。
- [改进] 多策略联合新增独立执行门控：以当前价、近端阻力、硬失效位计算盈亏比；接近阻力或盈亏比低于 1.5 时不追入，跌破硬失效位时回避/退出，且不篡改五维方向得分。
- [新功能] 多策略联合新增独立「仓位与分批执行」区块：以硬失效位和单笔风险公式定仓，计划仓位按 50%/30%/20% 金字塔分段，仅允许盈利且结构确认后加仓；同时展示 1R/2R 分批止盈、近端支撑—阻力推导的斐波那契回踩参考及数据不足时的新增仓位禁令。
- [改进] 问股在服务端接受提问并开始回传后，会清空并收回已手动拉高的输入框至紧凑默认高度；自动扩展、手动调节和 ArrowUp 回填保持不变。
- [改进] xiaozhaolucky 美股交易 skill 新增实盘轮动做 T 归纳、策略科学性审查、证据缺口、量化定义与样本外验证框架。
- [改进] 将启动时未配置 `STOCK_LIST` 的默认自选股及 `.env` 示例更新为当前持仓列表。
...
- [改进] 多策略联合输出升级为“结论摘要、五维证据汇总、13 项评分明细、评分审计”结构；保留逐项得分、权重与原始公式，改用去重后的五维综合得分驱动决策，新增证据质量/维度门控、空仓与持仓动作分离，并移除缺少风险参数时的固定重仓建议。
- [改进] 多策略联合评分表增加趋势、量价、结构、情绪、相对强弱五类维度；贪恐数据不可用时改按缺失证据排除，不再使用代理中性分，并明确综合分不是收益概率。
- [修复] 多策略五维方向出现高冲突时新增确定性阻断，统一降级为「证据冲突（观望）」并禁止强方向仓位建议。
- [改进] 多策略联合新增必调相对强弱工具：按共同交易日展示个股相对大盘及可验证行业基准的 5/20/60 日超额收益、来源与截止日期；行业基准缺失或过期时明确降级，不猜测龙头证据。
- [改进] 多策略确定性评分区块不再向最终报告输出 `multi-strategy-score` HTML 注释标记，内部仍保留标记清理能力。
- [改进] 多策略评分与看多/看空结论增加统一 🟢/🟡/🔴/⚪ 颜色标识和图例，综合得分、13 项明细与最终决策一眼可区分方向和数据缺失。
- [新功能] 多策略联合新增 Brooks-inspired 价格行为学证据视角，结构化展示趋势/区间、摆动结构、突破候选、信号 K 线、关键价位与二次入场确认边界。
- [改进] 将价格行为学升级为多策略第 13 项正式评分能力（权重 0.8、归入结构维度），同时继续作为趋势、放量突破、箱体震荡和缩量回踩的交叉证据；13 项完整总权重调整为 10.0。
- [新功能] 新增独立「价格行为学」问股策略入口，与多策略联合中的价格行为学子分析同时保留。
- [新功能] 价格行为学新增重要低点、ATR14 的 0.5-1.0 倍缓冲止损、强阳 K 移动止盈和真实 5m/15m VWAP Bands；分钟线使用 Longbridge 并在失败时降级到 Futu OpenD，日内数据缺失时明确降级且不使用日线代理值。
- [改进] 问股输入框支持在空输入状态按 ArrowUp 回填最近一次已发送的问题，编辑中的草稿和中文输入法候选不会被覆盖。
...
- [修复] 多策略联合报告改由后端根据评分工具结果自动附加权威 13 项评分区块，避免精简持仓报告因模型漏写策略行而耗尽 Agent 步骤；浪型解释门禁保持不变。
- [改进] 多策略联合新增 13 项证据分级与确定性评分校验，固定完整权重 10.0、缺失证据动态排除并报告覆盖率；第 3 浪候选/启动强制输出日期价位浪型图、替代数浪及确认/弱化/证伪条件。
...
- [修复] 当前股票分析增加最新数据硬门禁：本轮必须成功获取实时行情和网络刷新日线，禁止用历史对话或 DB 缓存冒充当前结论，核心数据源失败时直接停止生成最新走势分析。
...
- [改进] 强化阶段合理价格 skill 的模型置信度、质量警报、资产类型固定倍数框架和开发阶段自动分流规则。
- [修复] 美股估值分位在 yfinance `info` 瞬断或缺 PE/PB 字段时降级使用实时行情估值字段，减少问股分析中的非致命估值警告。
...
- [改进] `nunu_wave` 补充跳空高开与第五浪末端规则：跳空导致小级别浪型细节缺失时降低计数置信度，疑似第五浪第 4 子浪后再突破前高需警惕衰竭见顶，并将 8%/16% 作为小级别/放大级别回调经验阈值。
- [改进] `nunu_wave` 补充下跌推动浪截尾、大型平台/大型楔形双路径和失效位优先原则，要求每个主备计数都给出确认位、失效位与可证伪条件。
- [改进] `nunu_wave` 新增阶段快速四问，要求先检查前高突破、关键低点、量能扩张和日线/4H/30m 级别一致性，再决定是否允许高置信判定三浪启动。
- [改进] 问股输入区新增内置提示词模板，可一键填入通用股票分析、已持仓决策、价投分析和默认风控分析等常用问法，并自动切换可用策略但不自动发送。
- [改进] 收敛问股内置提示词模板的分析范围和输出长度，保留多策略联合调用同时减少新闻、研报、组合快照和多周期发散上下文。
- [改进] 问股「价值投资」策略和系统化价值投资报告新增护城河三重检验，逐项检查底层规律、结构互锁和飞轮自强化，并将三关结果纳入定价权与长期持有判断。
- [改进] 问股「价值投资」策略和系统化价值投资报告新增价值选股加分项与退出纪律，标注 Founder-led、核心位置、为什么是现在、五件事备忘和叙事溢价削减风险。
...
- [改进] `AGENT_ORCHESTRATOR_TIMEOUT_S` 未配置或非法值时的运行时默认由 600 秒调整为 **900** 秒（`src/config.py` + `config_registry` + `.env.example`），减轻问股多策略 / 大上下文 / 慢模型下的墙钟超时；仍可通过环境变量显式设为更小或 `0`（关闭限制）。
- [改进] 无 `stock_code` 时 LLM 市场语境默认改为美股 — `src/market_context.py` 的 `detect_market` 在代码为空时读取 `Config.default_market_context`（环境变量 `DEFAULT_MARKET_CONTEXT`，取值 `cn`/`hk`/`us`，默认 `us`）；有代码时仍按原规则识别。影响 Agent 问股、分析器注入的 `get_market_guidelines` 等；纯 A 股用户可在 `.env` 设 `DEFAULT_MARKET_CONTEXT=cn` 恢复旧行为。同步 `config_registry` / `.env.example` / `README` 配置表；新增 `tests/test_market_context.py`。
- [新功能] 长期价值投资能力补强 — 新增 `src/services/valuation_percentile_service.py`（A 股走 `akshare.stock_zh_valuation_baidu` 完整 5 年日频 PE/PB/PS 历史；美股 `yfinance.Ticker.info` 仅给当前值 status='partial'；港股 status='unavailable'，30 分钟 TTL 缓存，全部 fail-open）；同步新增 Agent 工具 `get_valuation_percentile`（params: stock_code, metric=pe/pb/ps, lookback_years，返回当前值 + 5 桶分位 + 中文评级 极低估/偏低估/合理/偏高估/极高估）；问股新增 3 个长期价值 skill —— 「💎 价值投资」(`value_investing`，巴菲特式 PE 分位 + 护城河 + 安全边际)、「🚀 高质量复利」(`quality_compounder`，Peter Lynch / Terry Smith GARP 风格 ROE 持续 + 营收 CAGR + 毛利)、「💵 股息成长」(`dividend_growth`，股息率 + payout 健康 + 连续增息 + FCF 覆盖)；前端按 `default_priority` 自动出现在策略栏（25/28/35 位次），无前端代码改动。新增 25 个单元测试覆盖市场检测 / 阈值分级 / A 股完整 / 美股 partial / 港股 unavailable / 错误代码 / 缓存复用。**完全增量改动**：不修改任何已有 yaml、tool、prompt、前端、`multi_strategy_consensus` 保持 12 项不变；不引入新数据源依赖（akshare/yfinance 已是 requirements）；不引入新配置项。
- [文档] `README.md` / `docs/README_CHT.md` / `docs/INDEX_EN.md` 顶部导航新增「能力总览 / 能力總覽 / Capabilities Map」入口，指向 `docs/capabilities/README.md`；英文索引明确标注「Chinese only at the moment」，避免英文用户误以为有英文版（按 AGENTS.md 双语文档同步说明）。
- [文档] 新增 `docs/capabilities/README.md` 能力总览（capability map）— 单页梳理系统当前能力：触发入口（CLI / Web / Bot / Schedule）、25 个 Skills、23 个 Agent Tools、10 个数据源 fallback 链、20+ services 导读、11 个通知渠道、API endpoints、机器人命令、10 条 CI/CD 工作流、外部依赖、报告类型、关键配置、扩展指南（加 skill / 工具 / 数据源 / 通知渠道的最小步骤）、维护参考；独立 `docs/capabilities/` 文件夹便于后续按主题拆分（agent-tools.md / strategies.md 等占位说明已写明）；本文件随用户可见能力变化同步更新。
- [改进] 问股页加载后默认选中「⚡ 多策略联合」(此前为「默认多头趋势」)；前端硬编码偏好 + 回退链（找不到 multi_strategy_consensus 时回退到后端 default_skill_id，再回退到首项），仅影响 Web 前端首次进入时的默认勾选，不修改后端 `/api/v1/agent/skills` 返回的 `default_skill_id`，桌面端 / API 文档等其它客户端默认行为不变。
- [新功能] 贪恐指数全面接入问股链路 — 新增 Agent 工具 `get_fear_greed_index`（`src/agent/tools/data_tools.py`），让问股 / 多策略联合等 chat 流程也能消费 szdt.tech 贪恐分（此前仅"正式分析"链路注入）；同步新增独立 skill `strategies/fear_greed_sentiment.yaml`（「😱 贪恐情绪」按钮，前端自动加载，`default_priority=30`），基于贪恐分做逆向情绪分析（极度贪婪警示高位 / 极度恐慌提示反向布局窗口）；`strategies/multi_strategy_consensus.yaml` 评估表从 11 项扩展为 12 项，第 12 项「贪恐情绪极值」权重 0.5 作为逆向指标参与加权综合得分；修正 `config_registry.py` 中 `SZDT_AUTH_TOKEN` 描述（实际为「已绑定股票数量」配额而非「每月 5000 次调用」），并说明账号股票池满后需手动释放。无 token 时返回 `status='not_configured'`，配额命中时返回 `status='unavailable'`，均不阻塞 LLM 后续分析。
- [新功能] 美股大盘复盘新增「💧 美股资金流动性面板」（`src/services/us_liquidity_service.py`）— 从 Yahoo Finance 拉取 5 个 Tier 1 流动性指标（VIX、^MOVE 债市波动率、^TNX 10Y 美债收益率、DX-Y.NYB 美元指数、HYG 高收益债 ETF），输出当前值 + 近 5 日变化 + 信号分级（🟢/🟡/🔴）+ 综合判断（偏宽松/偏紧缩/多空交织），自动附加到 `region in ('us', 'both')` 的大盘复盘报告中。零密钥、零外部依赖，failed-open（部分指标拉取失败时降级显示「数据不可用」，全部失败时静默跳过）；30 分钟 TTL 缓存避免重复请求。新增 `tests/test_us_liquidity_service.py`（13 个用例，mock yfinance 覆盖阈值分级、表格渲染、部分失败降级、缓存复用）。
