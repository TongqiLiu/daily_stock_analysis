...
| `AGENT_LLM_UPSTREAM_STREAM` | Consume upstream LLM output incrementally and aggregate it inside the Agent; reduces idle reverse-proxy resets during long inference | `false` | No |
...
| `AGENT_CONTEXT_SUMMARY_TIMEOUT_S` | Timeout in seconds for visible-history summarization, range `10-600` | `90` | No |
...
- `GET /api/v1/decision-signals/outcomes/stats`: aggregate current outcome-engine stats; by default it excludes archived signals and can isolate Ask Stock samples with `source_type=agent`.
- `POST /api/v1/decision-signals/outcomes/ai-review`: generate a read-only AI diagnosis from deterministic statistics; it never recomputes outcomes or changes weights.
...
- **Intraday T-trading Skill** - The Ask Stock selector includes `日内做T`, which always evaluates fresh 3-minute bars with EMA20/EMA50, ATR14, and HH/HL/LH/LL structure. It reduces T-trading in strong trends, enables it only after a range forms, requires LH plus EMA20 loss for a high sell and support plus HL plus EMA20 reclaim for a buyback, and rejects plans with less than 1 ATR of room while preferring at least 1.5 ATR.
- **Multi-strategy price structure** - The authoritative multi-strategy score block also shows current price location, nearest support, nearest resistance, breakout status, and hard invalidation level. These fields come from `analyze_trend`; missing data is marked explicitly rather than estimated by the model.
- **Options strategy analysis** - Ask Stock now includes `期权策略分析` and `期权组合分析` entries. Specific combinations use Futu `get_option_strategy_analysis` for provider-level bid/ask, max profit/loss, breakeven points, probability of profit, Delta, and Theta. Missing permissions or fields downgrade to an explicit data gap; prices and Gamma are never invented.
...
- **Ask-stock conversation copy** - The Copy button beside the input writes the current user and AI messages to the clipboard as Markdown; it is disabled for an empty chat and briefly changes to “Copied” after success.
...
| `/api/v1/decision-signals/outcomes` | GET | Paginated signal outcome query with stock, source, horizon, status, and result filters plus originating-signal audit context |
| `/api/v1/decision-signals/outcomes/stats` | GET | Query current outcome-engine stats; archived signals are excluded by default and `source_type=agent` isolates Ask Stock samples |
| `/api/v1/decision-signals/outcomes/ai-review` | POST | Generate a read-only AI diagnosis from deterministic outcome statistics |
...
> Note: `multi_strategy_consensus` is an exclusive meta-skill and cannot run alongside specialist skills such as Serenity Research or Value Investing. Selecting a specialist skill in the Web UI automatically clears the default consensus selection; if an API or older client submits a conflicting combination, the backend drops `multi_strategy_consensus` and keeps the specialist skills. Multiple specialist skills are executed in request order: their declared tools are completed first, then each skill receives a distinct result before synthesis. The latest request and current skill selection take precedence over scope restrictions from earlier turns unless the user repeats those restrictions. The consensus skill retains all 13 fixed weights (10.0 total), including Price Action at weight 0.8 in the structure dimension, and the raw weighted score for audit. Price Action uses key lows, ATR-buffered protection, strong bull trend bars, and provider-backed 5m/15m VWAP for invalidation and trailing exits. Intraday bars prefer Longbridge and fall back to Futu OpenD, with source, session, and as-of metadata; stale or missing data must not be replaced with a daily proxy. Correlated rows are first merged into trend (30%), price-volume (25%), structure (20%), relative strength (15%), and sentiment (10%); this five-dimension score drives the final direction and decision. The scoring tool validates non-overlapping signal/strength/score bands, excludes missing rows from the raw numerator and denominator, and computes quality coverage with `complete=1.0`, `partial=0.5`, and `missing=0`. The decision is forced to "insufficient evidence (watch)" when active-weight coverage is below 70%, quality coverage is below 65%, fewer than eight rows are evaluable, or fewer than four dimensions are available. After a successful score, the backend prepends an authoritative block with the conclusion, five-dimension evidence, Price Action risk controls, 13-row score detail, and score audit. Per-row scores, weights, and both formulas remain visible; flat-position and holding actions are separated, and position guidance uses stop distance plus account risk budget instead of fixed heavy-allocation percentages when those inputs are unavailable. The authoritative block also renders a separate position-and-tranche plan: the 50%/30%/20% split is a split of the intended position, not account equity; actual monetary size must be calculated from account risk budget and stop distance; adds require a profitable initial tranche plus structural confirmation; 1R/2R are staged-profit references; and Fibonacci levels are only support-to-resistance pullback observation zones, never automatic averaging-down triggers. Compact reports still retain this block and omit the `multi-strategy-score` HTML markers. Any Wave 3 candidate/start call must still include a dated and priced wave diagram, an alternative count, confirmation, weakening and hard-invalidation levels, and confidence. Low-volume breakouts, missing weekly evidence, or bearish long-term averages cannot receive high confidence.

> Weekly-data note: `get_weekly_history` prefers provider-native weekly bars from Futu OpenD when enabled and falls back to daily aggregation with explicit source, current-week partial-bar, and fallback-reason metadata. The current week must not be treated as a completed weekly bar.

> Multi-strategy relative-strength note: the system calculates matched-session 5/20/60-day excess returns against the broad-market benchmark and an optional verified sector benchmark, with benchmark codes, sources, and as-of dates. Missing or stale sector data remains partial evidence and is never inferred. High directional conflict across the five dimensions deterministically overrides any raw strong score with `conflicting evidence (watch)` and no new-position guidance.
